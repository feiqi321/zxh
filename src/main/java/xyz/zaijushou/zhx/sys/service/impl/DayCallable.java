...
public class DayCallable implements Callable<List<StatisticReturn2>> {
    ...
    public List<StatisticReturn2> call() throws Exception {
        ...
        for (String str : timeAreaAttr) {
            ...
            int countConPhoneNum = getNum(sdf2, sdf, conList, dateStart, dateEnd);
            int countInvalidPhoneNum = getNum(sdf2, sdf, invalidList, dateStart, dateEnd);
            int countCasePhoneNum = getIntegers(sdf2, sdf, caseList, sets, dateStart, dateEnd);
            col.setCountConPhoneNum(countConPhoneNum);
            col.setCountInvalidPhoneNum(countInvalidPhoneNum);
            col.setCountPhoneNum(countConPhoneNum+countInvalidPhoneNum);
            col.setCountCasePhoneNum(countCasePhoneNum);
            colList.add(col);
        }
        conInfo.setList(colList);
        conInfo.setSumConPhoneNum(conList.size());
        conInfo.setSumInvalidPhoneNum(invalidList.size());
        conInfo.setSumPhoneNum(conList.size()+invalidList.size());
    ...
    private int getIntegers(SimpleDateFormat sdf2, SimpleDateFormat sdf, List<CollectionStatistic> caseList, HashSet<Integer> sets, Date dateStart, Date dateEnd) throws ParseException, ParseException {
    ...
    private int getNum(SimpleDateFormat sdf2, SimpleDateFormat sdf, List<CollectionStatistic> listInfo, Date dateStart, Date dateEnd) throws ParseException {
        int num = 0;
